...
class SubitoParser:
    ...
    __PROXIES = {
        'https': f'http://user164993:21perz@181.215.227.162:2363'}
    ...
    async def __get_products_links(self, user_id, page_num):
        url = f'/annunci-italia/vendita/casa-e-persona/?shp=true&o={page_num}'
        ...
        if "You don't have permission to access" in html:
            raise ConnectionError("Включи впн")
        ...
        for ad in ads:
            ...
            status, user_url, ad_time = self.__get_user_url(ad_url)

            if status:
                text = (f'<a href="{ad_url}">Товар</a>\n'
                        f'<a href="{self.__DOMEN}{user_url}">Продавец</a>\n'
                        f'Время: {ad_time}')
                # print(f'{self.__DOMEN}{user_url}, {ad_time}')
                await self.__bot.send_message(user_id, text, parse_mode=ParseMode.HTML)

    def __get_user_url(self, ad_url: str) -> tuple:
        response = requests.get(
            ad_url,
            params=self.__PARAMS,
            cookies=self.__COOKIES,
        ...
        return self.__get_user_data(link), link, f'Сегодня в {time_[-1]}'

    def __get_user_data(self, url) -> bool:
        response = requests.get(
            url=self.__DOMEN + url,
            params=self.__PARAMS,
            cookies=self.__COOKIES,
